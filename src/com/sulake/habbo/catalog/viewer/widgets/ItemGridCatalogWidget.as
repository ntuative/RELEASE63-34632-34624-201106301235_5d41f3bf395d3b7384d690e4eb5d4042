   ...
   public class ItemGridCatalogWidget extends CatalogWidget implements ICatalogWidget, IItemGrid, IDragAndDropDoneReceiver
   {
      ...
      public function ItemGridCatalogWidget(param1:IWindowContainer, param2:ISessionDataManager)
      {
         ...
         if(this._scrollBar != null)
         {
            ...
            this._scrollBar.addEventListener(WindowEvent.const_893,this.activateScrollbar);
            this._scrollBar.addEventListener(WindowEvent.const_286,this.activateScrollbar);
            this._scrollBar.addEventListener(WindowEvent.const_995,this.deActivateScrollbar);
      ...
      protected function loadItemGridGraphics(param1:TimerEvent = null) : void
      {
         ...
         if(_loc2_ > 0)
         {
            ...
            while(_loc3_ < 3)
            {
               if(this.var_920 >= 0 && this.var_920 < _loc2_)
               {
                  _loc4_ = page.offers[this.var_920];
                  this.loadGraphics(_loc4_);
                  _loc4_.productContainer.grid = this;
               }
               ++this.var_920;
               if(this.var_920 >= _loc2_)
               {
                  this.resetTimer();
                  break;
               }
      ...
      {
         var _loc2_:IWindowContainer = page.viewer.catalog.windowManager.buildFromXML(this.var_1902) as IWindowContainer;
         ...
         if(param1.pricingModel == Offer.const_652)
         {
            ++this.var_2197;
            (param1.productContainer as BundleProductContainer).setBundleCounter(this.var_2197);
         }
      }
