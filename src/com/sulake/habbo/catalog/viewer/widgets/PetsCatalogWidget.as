package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget, IPetDataListener, IPetImageListener
   {
      ...
      override public function init() : Boolean
      {
         ...
         this.var_2657 = _loc3_.localizationId;
         this.var_413 = [];
         if(page && false && page.viewer.catalog)
         {
         ...
         events.addEventListener(WidgetEvent.const_798,this.onApproveNameResult);
      ...
      private function onWidgetsInitialized(param1:CatalogWidgetsInitializedEvent = null) : void
      {
         ...
         var _loc3_:* = [];
         for each(_loc4_ in this.var_413)
         {
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         this.var_578 = param1.index;
         if(this.var_578 < 0 || this.var_578 > this.var_413.length)
         {
            this.var_578 = 0;
         }
         this.updateImage();
      ...
      {
         if(param1.productCode == this.var_2657)
         {
      ...
      private function updateImage() : void
      {
         ...
         if(this.var_225 == null || this.var_884 >= this.var_225.length)
         {
            return;
         }
         var _loc2_:* = 0;
         if(this.var_578 >= 0 && this.var_578 < this.var_413.length)
         {
            _loc7_ = this.var_413[this.var_578] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:int = this.var_225[this.var_884] as int;
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImage(this.var_558,_loc3_,_loc2_,AvatarScaleType.const_61,this);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_321,AvatarAction.const_776);
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(this.var_578 >= this.var_413.length)
         {
            return "";
         }
         var petColor:IPetColor = this.var_413[this.var_578] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:int = this.var_225[this.var_884] as int;
