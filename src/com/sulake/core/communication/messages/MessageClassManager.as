   ...
   public class MessageClassManager implements IMessageClassManager
   {
      ...
      private function registerMessageEvent(param1:int, param2:Class) : Boolean
      {
         ...
         if(_loc3_..implementsInterface.@type == this._messageEventInterface)
         {
            ...
            {
               this.var_899[param1] = [param2];
            }
            else
            {
               _loc6_ = this.var_899[param1];
               _loc6_.push(param2);
            }
            return true;
         }
      ...
      public function getMessageComposerID(param1:IMessageComposer) : int
      {
         ...
         var _loc4_:* = null;
         var _loc2_:int = -1;
         for(_loc3_ in this.var_1111)
         {
            _loc4_ = this.var_1111[_loc3_] as Class;
            if(param1 is _loc4_)
            {
      ...
      {
         var _loc2_:Array = this.var_899[param1];
         if(_loc2_ != null)
         {
            return _loc2_;
         }
      ...
      public function toString() : String
      {
         ...
         for(_loc2_ in this.var_1111)
         {
            _loc1_ += _loc2_ + " -> " + this.var_1111[_loc2_] + "\n";
         }
         _loc1_ += "Registered Message Event Classes: \n";
         for(_loc2_ in this.var_899)
         {
            _loc1_ += _loc2_ + " -> " + this.var_899[_loc2_] + "\n";
         }
         return _loc1_;
      }
   }
