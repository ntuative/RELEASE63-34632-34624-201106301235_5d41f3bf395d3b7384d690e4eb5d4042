package com.sulake.habbo.communication.demo
{
   ...
   public class SSOTicket extends EventDispatcher
   {
      ...
      private var var_1542:IHabboWebLogin;
      ...
      public function SSOTicket(param1:IAssetLibrary, param2:IHabboWebLogin, param3:String, param4:Boolean = true)
      {
         ...
         this.var_1541 = "http://" + param3 + "/client";
         if(!param4)
         {
            this.var_1542.init();
            this.var_1542.addEventListener(HabboWeb.const_267,this.requestClientURL);
         }
         else
         {
      ...
      {
         var _loc2_:String = this.var_1541;
         if(this._assets.hasAsset(_loc2_))
         {
            Logger.log("[CoreLocalizationManager] reload localization for url: " + this.var_1541);
         }
         var _loc3_:URLRequest = new URLRequest(this.var_1541);
         if(this._assets.hasAsset(_loc2_))
         {
      ...
      private function clientURLComplete(param1:Event = null) : void
      {
         ...
         {
            this.var_1542.requestReAuthenticate();
         }
         else
         {
            try
            {
               this.var_1925 = /\"sso.ticket\" : \"(.*?)\"/.exec(data)[1];
               this.var_1813 = /\"flash.client.url\" : \"(.*?)\"/.exec(data)[1];
               facebookData = /\"facebook.user\" : \"(.*?)\"/.exec(data);
               if(facebookData)
               {
                  this.var_2601 = Boolean(facebookData[1]);
               }
               if(this.var_1925.length > 0)
               {
