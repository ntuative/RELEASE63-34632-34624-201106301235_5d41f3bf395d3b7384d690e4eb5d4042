package com.sulake.habbo.messenger.domain
{
   ...
   public class Conversations
   {
      ...
      private static var var_1398:int = 1;
       
      
      private var var_500:IConversationsDeps;
      ...
      public function setSelected(param1:Conversation) : void
      {
         ...
         var _loc2_:int = this.var_164.indexOf(param1);
         while(this._startIndex + this.var_500.getTabCount() <= _loc2_)
         {
            ++this._startIndex;
         }
      }
      ...
      public function closeConversation() : void
      {
         ...
         if(_loc1_ != null)
         {
            _loc2_ = this.var_164.indexOf(_loc1_);
            Util.remove(this.var_164,_loc1_);
            this._closedConversations.push(_loc1_);
            _loc1_.setSelected(false);
            while(_loc2_ >= 0)
            {
               if(this.var_164[_loc2_] != null)
               {
                  this.var_164[_loc2_].setSelected(true);
                  break;
               }
               _loc2_--;
            }
      ...
      {
         this.addConversation(var_1398++);
      }
      
      public function addConversation(param1:int) : Conversation
      {
         var _loc2_:Boolean = this._closedConversations.length == 0 && this.var_164.length == 0;
         var _loc3_:Conversation = this.addConversationInt(param1);
         if(_loc3_ == null)
         {
            return null;
         }
         if(this.var_164.length == 1)
         {
            this.var_164[0].setSelected(true);
         }
         if(_loc2_)
         {
            _loc3_.addMessage(new Message(Message.const_796,0,this.var_500.getText("messenger.moderationinfo"),""));
         }
         return _loc3_;
      }
      ...
      {
         var _loc2_:int = this.var_164.length;
         ...
         this.var_500.addMsgToView(_loc3_,param1);
         if(_loc2_ != this.var_164.length || _loc4_ != _loc3_.newMessageArrived)
         {
            this.var_500.refresh(false);
         }
      }
      ...
      public function setOnlineStatusAndUpdateView(param1:int, param2:Boolean) : void
      {
         ...
         this.var_500.addMsgToView(_loc3_,_loc4_);
      }
      ...
      private function addConversationInt(param1:int) : Conversation
      {
         ...
         if(_loc2_ != null)
         {
            ...
            this.var_164.push(_loc2_);
            return _loc2_;
         }
         _loc2_ = this.var_500.createConversation(param1);
         if(_loc2_ == null)
         {
            return null;
         }
      ...
      {
         this._startIndex = Math.min(this._startIndex,this.var_164.length - this.var_500.getTabCount());
         this._startIndex = Math.max(0,this._startIndex);
      }
      
      private function getOnlineInfoMsg(param1:Boolean) : Message
      {
         return new Message(Message.const_857,0,this.getOnlineText(param1),Util.getFormattedNow());
      }
      
      private function getOnlineText(param1:Boolean) : String
      {
         return this.var_500.getText(!!param1 ? "messenger.notification.online" : "messenger.notification.offline");
      }
   }
}
