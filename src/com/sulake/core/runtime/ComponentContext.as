package com.sulake.core.runtime
{
   ...
   public class ComponentContext extends Component implements IContext
   {
      ...
      protected function loadErrorHandler(param1:LibraryLoaderEvent) : void
      {
         ...
         this.removeLibraryLoader(_loc2_);
         this.error("Failed to download component resource \"" + _loc2_.url + "\"!" + "\r" + _loc3_,true,Core.const_1812);
      }
         ...
         while(i < componentList.length())
         {
            ...
            if(assetCount > 0)
            {
               ...
               while(k < assetCount)
               {
                  ...
                  {
                     this.error("Manifest for component " + componentString + " contains broken asset \"" + assetName + "\"!",true,Core.const_968);
                  }
                  k++;
               }
            }
            ...
            if(componentClass == null)
            {
               this.error("Invalid component class " + componentString + "!",true,Core.const_968);
               return null;
            }
            ...
            if(component != null)
            {
               if(assetLibrary != null)
               {
                  if(component.assets != assetLibrary)
                  {
                     ...
                     this.error("Component \"" + componentString + "\" did not save provided asset library!",true,Core.const_968);
                  }
               }
      ...
      protected final function announceInterfaceAvailability(param1:IID, param2:Component) : void
      {
         ...
         while(_loc6_ < _loc5_)
         {
            ...
            if(_loc4_ == null)
            {
               this.error("Interface " + param1 + " still unavailabe!",true,Core.const_1879);
            }
