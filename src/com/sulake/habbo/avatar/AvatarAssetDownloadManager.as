package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private var var_972:int = 3;
      
      private var var_447:AssetLoaderStruct;
      
      private var var_974:Timer;
      ...
      private const const_2345:int = 100;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         this.var_974 = new Timer(this.const_2345,1);
         this.var_974.addEventListener(TimerEvent.TIMER_COMPLETE,this.onNextDownloadTimeout);
      }
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_447)
         {
            ...
            this.var_447.addEventListener(AssetLoaderEvent.const_41,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_447)
         {
            ...
            this.var_447.removeEventListener(AssetLoaderEvent.const_41,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         if(this.var_972 <= 0)
         {
            HabboWebTools.logEventLog("Figuremap download error " + this.var_1181);
            Logger.log("AVATAR ASSET ERROR: Could not download figuremap.xml.");
         }
         else
         {
            Logger.log("Could not download figuremap.xml. Retries left: " + this.var_972);
            if(this.var_1181.indexOf("?") > 0)
            {
               _loc2_ = this.var_1181 + "&retry=" + this.var_972;
            }
            else
            {
               _loc2_ = this.var_1181 + "?retry=" + this.var_972;
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2367,this._assets);
      ...
      public function loadFigureSetData(param1:IAvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_1770)
         {
            this.var_1180.push([param1,param2]);
            return;
         }
         ...
         if(_loc4_.length > 0)
         {
            ...
            {
               _loc6_ = this.var_567[_loc3_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(param2);
               this.var_567[_loc3_] = _loc6_;
            }
            this.var_973[_loc3_] = _loc4_;
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         for(_loc3_ in this.var_973)
         {
            ...
            if(_loc7_)
            {
               ...
               delete this.var_567[_loc3_];
            }
         }
         for each(_loc3_ in _loc2_)
         {
            delete this.var_973[_loc3_];
         }
      ...
      {
         var _loc1_:* = null;
         while(this.var_777.length > 0 && this.var_776.length < this.const_1921)
         {
            _loc1_ = this.var_777[0];
            _loc1_.startDownloading();
            this.var_776.push(this.var_777.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_777.indexOf(param1) == -1 && this.var_776.indexOf(param1) == -1)
         {
