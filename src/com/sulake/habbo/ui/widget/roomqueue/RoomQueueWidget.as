package com.sulake.habbo.ui.widget.roomqueue
{
   ...
   public class RoomQueueWidget extends RoomWidgetBase
   {
      ...
      override public function registerUpdateEvents(param1:IEventDispatcher) : void
      {
         ...
         param1.addEventListener(RoomWidgetRoomQueueUpdateEvent.const_449,this.onQueueStatus);
      ...
      override public function unregisterUpdateEvents(param1:IEventDispatcher) : void
      {
         ...
         param1.removeEventListener(RoomWidgetRoomQueueUpdateEvent.const_449,this.onQueueStatus);
      ...
      private function onQueueStatus(param1:RoomWidgetRoomQueueUpdateEvent) : void
      {
         ...
         this.var_2692 = param1.hasHabboClub;
         this.var_1995 = param1.isClubQueue;
         localizations.registerParameter("room.queue.position","position",this.var_1292.toString());
         localizations.registerParameter("room.queue.position.hc","position",this.var_1292.toString());
         localizations.registerParameter("room.queue.spectator.position","position",this.var_1292.toString());
         localizations.registerParameter("room.queue.spectator.position.hc","position",this.var_1292.toString());
         this.showInterface();
      }
      ...
      private function showInterface() : void
      {
         ...
         {
            switch(this.var_1994)
            {
               case RoomWidgetRoomQueueUpdateEvent.const_449:
                  _loc1_.caption = !!this.var_1995 ? "${room.queue.position.hc}" : "${room.queue.position}";
                  _loc2_.caption = "${room.queue.spectatormode}";
                  _loc3_.visible = this.var_1578;
                  break;
               case RoomWidgetRoomQueueUpdateEvent.const_496:
                  _loc1_.caption = !!this.var_1995 ? "${room.queue.spectator.position.hc}" : "${room.queue.spectator.position}";
      ...
      private function exitQueue(param1:WindowMouseEvent) : void
      {
         ...
         var _loc2_:RoomWidgetRoomQueueMessage = new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.const_925);
      ...
      private function changeQueue(param1:WindowMouseEvent) : void
      {
         ...
         if(this.var_1994 == RoomWidgetRoomQueueUpdateEvent.const_449)
         {
            _loc2_ = new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.CHANGE_TO_SPECTATOR_QUEUE);
         }
         else
         {
            _loc2_ = new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.const_979);
         }
         messageListener.processWidgetMessage(_loc2_);
         this.removeWindow();
      }
