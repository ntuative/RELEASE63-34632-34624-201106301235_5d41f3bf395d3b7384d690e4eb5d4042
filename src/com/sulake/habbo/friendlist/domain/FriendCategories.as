package com.sulake.habbo.friendlist.domain
{
   ...
   public class FriendCategories
   {
      ...
      private var var_500:IFriendCategoriesDeps;
      ...
      public function onFriendListUpdate(param1:IMessageEvent) : void
      {
         ...
         try
         {
            ...
            for each(updatedFriend in p.updatedFriends)
            {
               ...
               this.var_500.messenger.setFollowingAllowed(updatedFriend.id,updatedFriend.followingAllowed && updatedFriend.online);
               currentlyOnline = this.isFriendOnline(updatedFriend.id);
               if(currentlyOnline && !updatedFriend.online)
               {
                  this.var_500.messenger.setOnlineStatus(updatedFriend.id,updatedFriend.online);
               }
               if(!currentlyOnline && updatedFriend.online)
               {
                  this.var_500.messenger.setOnlineStatus(updatedFriend.id,updatedFriend.online);
                  this.var_500.view.setNewMessageArrived();
               }
      ...
      private function updateCategories(param1:Array) : void
      {
         ...
         this.findCategory(FriendCategory.const_1264).received = true;
