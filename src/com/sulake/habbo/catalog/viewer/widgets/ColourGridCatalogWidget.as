package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class ColourGridCatalogWidget extends CatalogWidget implements ICatalogWidget
   {
      ...
      override public function init() : Boolean
      {
         ...
         this.var_2155 = _loc1_.content as XML;
      ...
      private function onAvailableColours(param1:CatalogWidgetColoursEvent) : void
      {
         ...
         this.select(this.var_420.getGridItemAt(param1.index) as IWindowContainer);
      }
      ...
      private function onAvailableMultiColours(param1:CatalogWidgetMultiColoursEvent) : void
      {
         ...
         for each(_loc2_ in param1.colours)
         {
            this.var_1116.push(_loc2_.slice());
         }
         _loc3_ = page.viewer.catalog.assets.getAssetByName(param1.backgroundAssetName) as BitmapDataAsset;
         this.var_731 = _loc3_.content as BitmapData;
         _loc4_ = page.viewer.catalog.assets.getAssetByName(param1.colourAssetName) as BitmapDataAsset;
      ...
      private function populateColourGrid() : void
      {
         ...
         var _loc12_:int = 0;
         var _loc13_:int = 0;
         Logger.log("[ColourGridCatalogWidget] Display colors: " + [this.var_1116.length,this.var_420.numGridItems]);
         this.var_420.destroyGridItems();
         this.var_239 = null;
         for each(_loc1_ in this.var_1116)
         {
            if(_loc1_.length > 0)
            {
               _loc2_ = page.viewer.catalog.windowManager.buildFromXML(this.var_2155) as IWindowContainer;
               _loc2_.addEventListener(WindowMouseEvent.WINDOW_EVENT_MOUSE_CLICK,this.onClick);
               _loc2_.background = true;
               _loc2_.color = 4294967295;
               _loc2_.width = this.var_731.width;
               _loc2_.height = this.var_731.height;
               this.var_420.addGridItem(_loc2_);
               _loc3_ = _loc2_.findChildByTag("BG_BORDER") as IBitmapWrapperWindow;
               if(_loc3_ != null)
               {
                  _loc3_.bitmap = new BitmapData(this.var_731.width,this.var_731.height,true,0);
                  _loc3_.bitmap.copyPixels(this.var_731,this.var_731.rect,new Point(0,0));
               }
               _loc4_ = _loc2_.findChildByTag("COLOR_IMAGE") as IBitmapWrapperWindow;
               if(_loc4_ != null)
               {
                  _loc4_.bitmap = new BitmapData(this.var_1117.width,this.var_1117.height,true,0);
                  _loc6_ = 255;
                  _loc7_ = 255;
                  _loc8_ = 255;
                  _loc9_ = null;
               ...
               if(_loc5_ != null)
               {
                  _loc5_.bitmap = new BitmapData(this.var_1002.width,this.var_1002.height,true,16777215);
                  _loc5_.bitmap.copyPixels(this.var_1002,this.var_1002.rect,new Point(0,0),null,null,true);
